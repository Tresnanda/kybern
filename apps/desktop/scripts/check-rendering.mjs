...
try {
  ...
  const build = spawnSync("pnpm", ["exec", "vite", "build", "--config", "perf/vite.config.ts", "--outDir", dist], { cwd: desktop, encoding: "utf8" })
  ...
  const result = spawnSync("swift", [path.join(desktop, "scripts/check-rendering.swift"), dist, csp], { stdio: "inherit" })
