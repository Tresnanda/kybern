...
// Keep Codex's original curve anchors even though the default presets start at zero.
...
export const CODE_THEME_OPTIONS: readonly CodeThemeOption[] = [
  ...
  { id: "kybern", label: "Kybern", variants: ["light", "dark"] },
...
export function normalizeCodeThemeId(
  ...
  const rawCodeThemeId = typeof codeThemeId === "string" ? codeThemeId.trim().toLowerCase() : "";
  // Stored states from before the preset rename.
  const normalizedCodeThemeId = rawCodeThemeId === "synara" ? "kybern" : rawCodeThemeId;
