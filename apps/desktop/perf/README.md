...
| Tall mermaid diagram layer | Hosted markdown promoted wrapping `.chat-mermaid` as one layer as tall as the source. Preview images are already capped at 24rem. | Skip the wrapper; host the bounded preview image and fallback note. Native A/B: `KYBERN_PERF_LIVE_MERMAID_LAYER=1`. |
...
See [tall mermaid diagram layer](hosted-mermaid-2026-09-18.md) for why hosted
markdown must not promote wrapping `.chat-mermaid`, and the native A/B control
(`KYBERN_PERF_LIVE_MERMAID_LAYER`).
