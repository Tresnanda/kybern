...
| Hidden environment composer stacks | A second WebContent process kept queued prompts, approvals, questions, and landing-tray DOM while occluded or minimized. Unfocused but visible windows must not drop those panels. | Unmount reconstructible `above` stacked panels while the window is occluded/minimized/page-hidden; keep the composer input so drafts and attachment previews survive. Blur is not discard. Verify two-window `vmmap` on macOS. |
...
See [hidden-window composer stacked-panel release](hidden-window-composer-2026-09-18.md)
for occlusion-gated unmount of queued/approval/question rails (not blur). The
composer input stays mounted. Linux cannot run the two-window macOS `vmmap` check.
...
| Occluded or minimized window with queued prompts or an approval | Two-window `vmmap` on macOS 27 plus stacked-panel remount; do not treat blur as compact permission. See `hidden-window-composer-2026-09-18.md`. |
