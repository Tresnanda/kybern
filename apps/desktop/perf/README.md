...
| Hidden environment sidebar lists | A second WebContent process kept project/thread rows while occluded or minimized. Unfocused but visible windows must not drop that list. | Unmount reconstructible sidebar rows while the window is occluded/minimized/page-hidden; keep the drag-region header. Blur is not discard. Verify two-window `vmmap` on macOS. |
...
See [hidden-window sidebar list release](hidden-window-sidebar-2026-09-18.md)
for occlusion-gated unmount of reconstructible project/thread rows (not blur).
...
| Occluded or minimized window with a populated sidebar | Two-window `vmmap` on macOS 27 plus sidebar remount; do not treat blur as compact permission. See `hidden-window-sidebar-2026-09-18.md`. |
