...
| Inline edited-files card layer | The “Edited N files” card hosted the whole expanded diff as one layer past 1024 CSS px. | Leave `[data-edited-files]` unhosted; host the header and each diff `tr`. Native A/B: `KYBERN_PERF_LIVE_EDITED_FILES_LAYER=1`. |
...
See [inline edited-files card layer](edited-files-layer-2026-09-18.md) for why
the transcript “Edited N files” card must not promote an expanded diff, and
the native A/B control (`KYBERN_PERF_LIVE_EDITED_FILES_LAYER`).
