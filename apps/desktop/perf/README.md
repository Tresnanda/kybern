...
| Remote https rasters | Visible 280px chips decoded the full remote `https?` image. | Daemon 560×352 preview; renderer does not `fetch` remote bytes into a canvas. Dialog/copy/download keep the original URL. |
...
See [remote https preview](remote-https-preview-2026-09-18.md) for 280px
`https?` chips fitted via a daemon 560×352 preview. The renderer does not
`fetch` remote bytes into a canvas. This Linux environment cannot reproduce
the macOS whole-app physical-footprint coalition.
...
| Image previews, local links, image recovery, remote https chip identity | `node scripts/check-rendering.mjs artifacts` |
