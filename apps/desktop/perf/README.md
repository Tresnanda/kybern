...
| Hosted markdown paragraph layer | Hosted `> *` promoted wrapping `p`/`h*`/`hr`; a huge paste as one paragraph tiled past 1024 CSS px. | Skip those wrappers; host `.chat-prose-chunk` (split past 1600 characters) and `hr`. Native A/B: `KYBERN_PERF_LIVE_PARAGRAPH_LAYER=1`. |
...
See [hosted markdown paragraph layer](paragraph-layer-2026-09-18.md) for why
ordinary `p`/`h*`/`hr` must not stay one leftover hosted wrapper, and the
native A/B control (`KYBERN_PERF_LIVE_PARAGRAPH_LAYER`).
