...
| Hidden environment card | A second WebContent process kept git/diff/notes rows in the floating environment card while occluded or minimized, including when the card was already faded for an unfocused window. | Unmount the reconstructible body while the window is occluded/minimized/page-hidden; keep the wrapper. Blur is not discard. Verify two-window `vmmap` on macOS. |
...
See [hidden-window environment card release](hidden-window-environment-2026-09-18.md)
for occlusion-gated unmount of the reconstructible environment card body (not blur).
Linux cannot run the two-window macOS `vmmap` check.
...
| Occluded or minimized window with the environment card | Two-window `vmmap` on macOS 27 plus environment-card remount; do not treat blur as compact permission. See `hidden-window-environment-2026-09-18.md`. |
