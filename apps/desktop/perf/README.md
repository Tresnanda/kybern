...
| Assistant answer paint hosts | Settled assistant bodies wrapped the whole answer in `.chat-paint-host` twice after Markdown blocks, images, and meta were already hosted. | Leave `[data-answer-host]` unhosted; nested markdown / images / meta stay hosted. Native A/B: `KYBERN_PERF_LIVE_ANSWER_LAYER=1`. |
...
See [assistant answer paint hosts](answer-paint-host-2026-09-18.md) for why
settled assistant bodies must not promote the whole Markdown tree, and the
native A/B control (`KYBERN_PERF_LIVE_ANSWER_LAYER`).
