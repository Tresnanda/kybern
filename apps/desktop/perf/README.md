...
| Opaque composer glass layer | Opaque mode set `--composer-glass-filter: none` but still generated a full-surface `::before` with `z-index: -1` and a specified `-webkit-backdrop-filter`. | Omit that pseudo when the window is opaque or reduced-transparency. Translucent mode keeps the one blur layer. Native A/B: `KYBERN_PERF_LIVE_COMPOSER_GLASS_LAYER=1`. |
...
See [opaque composer glass layer](composer-glass-layer-2026-09-18.md) for why
opaque mode still generated a composer `::before` stacking context after the
filter token was `none`, and the native A/B control
(`KYBERN_PERF_LIVE_COMPOSER_GLASS_LAYER`).
