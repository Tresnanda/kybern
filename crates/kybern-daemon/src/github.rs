...
async fn run(cwd: &Path, program: &str, args: &[&str]) -> Result<String> {
    let out = Command::new(program).current_dir(cwd).args(args).stdin(std::process::Stdio::null()).output().await.with_context(|| format!("run {program}"))?;
...
pub async fn status(cwd: &Path) -> Result<GitStatus> {
    if !Repo::is_repo(cwd).await {
        return Ok(GitStatus { is_git: false, branch: None, dirty_files: 0, ahead: 0, behind: 0, upstream: None, remote_url: None, pull_request: None });
    }
    ...
    let pull_request = match (&branch, remote_url.as_deref().is_some_and(|u| u.contains("github.com")), gh_available().await) {
        (Some(_), true, true) => run(cwd, "gh", &["pr", "view", "--json", "number,title,url,state,headRefName,baseRefName,isDraft,author,updatedAt"])
            .await
            .ok()
...
pub async fn diff_against_base(cwd: &Path, base: &str) -> Result<String> {
    let merge_base = run(cwd, "git", &["merge-base", &format!("origin/{base}"), "HEAD"]).await.or_else(|_| Ok::<_, anyhow::Error>(base.to_string()))?;
...
pub async fn pr_list(cwd: &Path, state: &str, limit: u32) -> Result<Vec<PullRequest>> {
    ...
    let json = run(cwd, "gh", &["pr", "list", "--state", state, "--limit", &limit, "--json", "number,title,url,state,headRefName,baseRefName,isDraft,author,updatedAt"]).await?;
