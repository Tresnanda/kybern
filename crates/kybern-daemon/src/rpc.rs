    ...
    match method {
        ...
        ThreadsGet::NAME => {
            ...
            if p.include_tool_output.unwrap_or(true) {
                state.store.hydrate_tool_outputs(id, &mut transcript).map_err(internal)?;
            }
        ...
        ThreadsToolOutput::NAME => {
            ...
            if state.store.thread_get(p.thread_id).map_err(internal)?.is_none() {
                return Err(RpcError::not_found("thread"));
            }
            let (output, is_error) = state
                .store
                .tool_call_output(p.thread_id, &p.tool_call_id)
                .map_err(internal)?
