...
pub fn render(ws: &mut Workspace, _window: &mut Window, cx: &mut Context<Workspace>) -> AnyElement {
    ...
    let providers: Vec<(ProviderKind, bool, Vec<PermissionMode>)> =
        ws.model.providers.iter().map(|p| (p.kind, p.available, p.supported_permission_modes.clone())).collect();
    let supported_modes: Vec<PermissionMode> =
        providers.iter().find(|(k, _, _)| *k == provider).map(|(_, _, m)| m.clone()).unwrap_or_else(|| PermissionMode::ALL.to_vec());
    ...
    let provider_button = Button::new("provider")
        ...
        .dropdown_menu(move |menu: PopupMenu, window, cx| {
            ...
            for (kind, available, _) in providers.clone() {
                menu = menu.item(PopupMenuItem::new(kind.display_name()).checked(kind == provider).disabled(!available).on_click(
                    window.listener_for(&workspace_entity(cx), move |this: &mut Workspace, _, _, cx| {
    ...
    let mode_button =
        Button::new("mode").ghost().xsmall().label(mode_label(mode)).dropdown_caret(true).tooltip("Permission mode").dropdown_menu(
            move |menu: PopupMenu, window, cx| {
                let mut menu = menu.min_w(px(200.));
                for m in PermissionMode::ALL {
                    let supported = supported_modes.contains(&m);
                    menu = menu.item(PopupMenuItem::new(mode_label(m)).checked(m == mode).disabled(!supported).on_click(
                        window.listener_for(&workspace_entity(cx), move |this: &mut Workspace, _, _, cx| this.set_thread_mode(m, cx)),
                    ));
                }
                menu
            },
        ...
        .child(
            v_flex()
                ...
                .when(!attachments.is_empty(), |el| {
                    el.child(h_flex().px_3().pt_3().gap_2().flex_wrap().children(attachments.into_iter().map(|(id, name)| {
                        h_flex().gap_1().px_2().py_1().rounded(px(6.)).bg(cx.theme().muted).text_xs().child(name).child(
                            Button::new(ElementId::Name(format!("rm:{id}").into())).ghost().xsmall().icon(IconName::Close).on_click(
                    ...
                    h_flex()
                        ...
                        .child(
                            h_flex()
                                .gap_2()
                                .items_center()
                                .when(running, |el| el.child(div().text_xs().text_color(muted).child("Working")))
                                .child(primary),
                        ),
                ),
        )
