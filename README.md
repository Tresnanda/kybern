...
fix needs a mobile build, with no desktop version bump. A change spanning the
...
Mobile builds are currently manual; no mobile EAS workflow or EAS Update channel
is configured. JavaScript changes reach Expo Go/development apps through Metro,
but an installed preview APK needs another APK until EAS Update is explicitly
set up. Native changes always require a new binary. Do not introduce mobile
