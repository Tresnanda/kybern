...
[Download desktop](https://github.com/Tresnanda/kybern/releases/latest) ·
[Download Android APK (preview)](https://github.com/Tresnanda/kybern/releases/download/android-build-7/kybern-0.1.1-android-build-7.apk)
...
| Android phone | [Download the preview APK](https://github.com/Tresnanda/kybern/releases/download/android-build-7/kybern-0.1.1-android-build-7.apk), then pair with a running Kybern daemon. No Metro or Expo Go required. |
| iPhone or iPad | The [mobile companion](apps/mobile/README.md), built with Apple device signing and paired with a running Kybern daemon. |
...
Open the downloaded APK on your phone and allow installation from your browser
when Android asks. It includes its JavaScript and assets and runs without Metro
or Expo Go. See the [Android release notes and checksum](https://github.com/Tresnanda/kybern/releases/tag/android-build-7).
This is a preview build, not a Google Play release. EAS builds the signed APK;
GitHub Releases hosts the public download.
...
See the [mobile update commands](apps/mobile/README.md#eas) and
[Android publishing steps](apps/mobile/README.md#publish-an-android-preview).
Android previews use `android-build-<versionCode>` tags and are always prereleases
with `--latest=false`. These tags intentionally contain no dotted version, so
they cannot match the daemon or desktop tag triggers, even on older build commits.
The mobile version appears in the release title and APK filename. Do not introduce
dotted mobile version tags without first narrowing the daemon workflow's broad trigger.
